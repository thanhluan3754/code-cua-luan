...
// --- KHAI BÁO HÀM (ĐỂ TRÁNH LỖI) ---
void printWordSmart(const char* text);
void showLoadingEffect(); // <--- Hàm mới cho hiệu ứng loading
...
void showLoadingEffect() {
  // Thanh loading rộng 100px, cao 10px, nằm giữa màn hình
  int barWidth = 100;
  int barHeight = 8;
  int x = (SCREEN_WIDTH - barWidth) / 2; // Căn giữa ngang (128-100)/2 = 14
  int y = 40; // Vị trí dọc

  for(int i = 0; i <= 100; i+=2) { // Chạy từ 0% đến 100%, bước nhảy 2
    display.clearDisplay();

    // Hiện chữ "LOADING..." ở trên
    display.setTextSize(1);
    display.setCursor(35, 20);
    display.print(F("LOADING..."));
    
    // Hiện số phần trăm bên dưới
    display.setCursor(55, 52);
    display.print(i);
    display.print("%");

    // Vẽ khung viền (Hollow Rectangle)
    display.drawRect(x, y, barWidth, barHeight, SSD1306_WHITE);

    // Vẽ phần đã load (Filled Rectangle)
    // Chiều dài phần fill = (i * chiều_dài_max) / 100
    // Trừ đi 2 pixel mỗi bên để tạo khoảng hở viền cho đẹp
    ...
    delay(30); // Tốc độ chạy loading (càng nhỏ càng nhanh)
  }
  
  delay(500); // Dừng lại 0.5s ở 100% cho người dùng kịp nhìn
  display.clearDisplay();
  display.display();
}